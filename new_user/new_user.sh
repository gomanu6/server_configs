...
# Chown Users Directory (Mountpoint)
# chmod users directory (Mountpoint)
...
function add_new_user () {
    ...
    if useradd --home "${users_dir}" --shell "${samba_users_shell}" -G "${samba_users_group}" "${username}"; then
        ...
        echo "[$(date +%Y%m%d_%H%M)add_new_user]: WARNING !! There was an error in adding ${username} to the system"
        exit 1
    fi
    ...
    if [ ! -d "${users_dir}" ]; then
        ...
        if mkdir -vp "${users_dir}"; then
            ...
            echo   "[$(date +%Y%m%d_%H%M)add_new_user]: WARNING !! Problem creating Mount Point"
            exit 1
        fi

    else
        echo "[$(date +%Y%m%d_%H%M)add_new_user]: WARNING !! User Directory already exists. Please verify and rectify"
        exit 1
    fi
...
EOF
    ...
    if [ -f "${samba_global_config_file}" ]; then
            echo "[$(date +%Y%m%d_%H%M)add_new_user]: Samba Config File exists....Making a backup"
            ...
            if mkfs.ext4 -v -L "${username}" "${lv_path}"; then
                ...
                if cp -v "${fstab_file}" "${fstab_backups_dir}fstab.backup.${backup_stamp}"; then
                    ...
                    if mount -v "${lv_path}" "${users_dir}"; then
                        echo "[add_new_user]: LVM for User Directory has been mounted successfully"
                        
                    else
                        echo "[add_new_user]: WARNING !! Unable to mount User LVM"
                    fi



                else
                    echo "[add_new_user]: WARNING!! Unable to create Backup of fstab file. Exiting"
                    exit 1
                fi

              
            else
                echo "[add_new_user]: WARNING Problem Formatting new LV"
                exit 1
            fi
...
add_new_user "${user_name}" "${user_pass}" | tee -a "${log_file}"